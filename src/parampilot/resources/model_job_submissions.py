"""Explicit asynchronous Train, Ask, and Predict job submissions."""
...
from parampilot.job_waiting import AsyncProgressCallback
...
from parampilot.resources.model_job_waiting import ModelJobWaitingMethods
from parampilot.serialization import public_id

ResultT = TypeVar("ResultT", TrainResult, AskResult, PredictResult)


class ModelJobSubmissionMethods(ModelJobWaitingMethods):
    """Typed submissions with one visibly train-capable method."""
    ...
    async def train_model(
        ...
        *,
        ...
        Args:
            ...
            wait: Await and return the terminal Train result when true.
            timeout: Positive local wait timeout or ``None``.
            poll_interval: Client minimum seconds between observations.
            on_progress: Optional validated sync-or-awaitable callback.
    ...
    async def create_ask_job(
        ...
        *,
        ...
        Args:
            ...
            idempotency_key: Optional logical job-submission key.
            wait: Await and return the terminal Ask result when true.
            timeout: Positive local wait timeout or ``None``.
            poll_interval: Client minimum seconds between observations.
            on_progress: Optional validated sync-or-awaitable callback.
            cancel_remote: Explicitly cancel if local waiting terminates.
    ...
    async def create_prediction_job(
        ...
        *,
        ...
        Args:
            ...
            idempotency_key: Optional logical job-submission key.
            wait: Await and return the terminal Predict result when true.
            timeout: Positive local wait timeout or ``None``.
            poll_interval: Client minimum seconds between observations.
            on_progress: Optional validated sync-or-awaitable callback.
            cancel_remote: Explicitly cancel if local waiting terminates.
    ...
    async def _submit(
        ...
        *,
        ...
        Args:
            ...
            wait: Whether to await terminal completion.
            timeout: Positive local wait timeout or ``None``.
            poll_interval: Client minimum seconds between observations.
            on_progress: Optional validated sync-or-awaitable callback.
            cancel_remote: Explicitly cancel if local waiting terminates.
