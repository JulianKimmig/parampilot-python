"""Synchronous campaign collaborator grant operations."""
...
class SyncCampaignAccessResource(SyncResource):
    """Typed synchronous campaign collaborator grant resource."""
    ...
    def iterate(
        ...
        *,
        limit: int = 100,
    ) -> Iterator[CampaignAccessGrantResponse]:
        ...
        Returns:
            Lazy synchronous access-grant iterator.

        """

        def fetch(cursor: str | None) -> CampaignAccessGrantPageResponse:
            ...
            """
            return self.list(campaign_id, limit=limit, cursor=cursor)

        return iterate_cursor_sync(fetch, operation_id="listCampaignAccessGrants")
