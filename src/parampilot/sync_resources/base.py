"""Typed request, response, metadata, and capability helpers for sync resources."""
...
T = TypeVar("T")
CapabilityChecker = Callable[[Collection[str]], object]


class SyncResource:
    """Base for cohesive sync resources using generated operation metadata.

    Args:
        transport: Reusable authenticated synchronous transport.
    ...
    def _require(self, capability: str | None) -> None:
        ...
        if capability is not None:
            self._capability_checker({capability})
    ...
    def _response(
        ...
        *,
        ...
        response = self._transport.request_operation(
            metadata,
    ...
    def _model(
        ...
        **kwargs: Any,
        ...
        """
        response = self._response(operation_id, adapter, **kwargs)
        if response is None:
            raise ResponseValidationError(
    ...
    def _empty(
        ...
        *,
        ...
        """
        metadata = operation(operation_id)
        self._transport.request_operation(
            metadata,
            metadata.path(**dict(path_values or {})),
            headers=headers,
        )
    ...
    def _download(
        ...
        *,
        ...
        Returns:
            Caller-owned synchronous download handle.

        """
        self._require(capability)
        metadata = operation(operation_id)
        response = self._transport.stream_operation(
            metadata,
            metadata.path(**dict(path_values or {})),
            params=params,
            json_body=json_value(body) if body is not None else None,
    ...
    def _required_download(self, operation_id: str, **kwargs: Any) -> Download:
        ...
        """
        download = self._download(operation_id, **kwargs)
        if download is None:
            raise ResponseValidationError(
