"""Buffered HTTPX request execution for the asynchronous SDK transport."""
...
async def send_buffered_response(
    *,
    ...
    """Execute buffered request attempts with one stable logical identity.
    ...
    for attempt in range(configuration.max_retries + 1):
        ...
        if retryable and error.retryable and attempt < configuration.max_retries:
            await _delay(configuration, attempt, error)
            continue
        raise error
