"""Unbuffered HTTPX response opening for the synchronous SDK transport."""
...
def open_stream_response(
    *,
    ...
    """Open a successful native sync stream with conservative retries.
    ...
    for attempt in range(configuration.max_retries + 1):
        ...
        if (
            ...
            _delay(configuration, attempt, error.retry_after)
            continue
        raise error
